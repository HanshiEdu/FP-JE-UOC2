...
public fun guardarPartida(
    db: SQLiteDatabase,
    nombre: String, monedas: Int, rachas: Int
): Single<Long> {
...
public fun UltimaFila (db: SQLiteDatabase): Jugador {
    val cursor = db.rawQuery("SELECT nombre, monedas, rachas, fecha FROM partida ORDER BY fecha DESC LIMIT 1", null)
    return if (cursor.moveToFirst()) {
        ...
        cursor.close()
        Jugador(title, monedas, rachas, fecha)
    } else {
        cursor.close()
        Jugador("manolo", 20, 0)
    }
